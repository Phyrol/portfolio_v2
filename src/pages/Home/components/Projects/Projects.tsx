...
const Projects = () => {
    ...
    return (
        <section>
            {!isWide && <h2 className="section-header mb-8">{HomePageSection.PROJECTS}</h2>}
            ...
            <div className="grid grid-cols-[repeat(auto-fit,_minmax(min(300px,_100%),_1fr))] gap-5">
                {PROJECT_INFO.map(({title, skills, details, position, downloadLink, thumbnailImage}) => (
                    <TiltElement key={title} enableGlare>
                        ...
                        >
                            <div className="bg-lightest-space-cadet/50 inset-ring-red-pantone/50 flex h-full flex-col rounded-lg inset-ring-1">
                                <div className="ring-red-pantone/80 aspect-[6/3] overflow-hidden p-3.5 md:aspect-[16/9]">
                                    <img
                                        src={thumbnailImage}
                                        alt={`${title} thumbnail`}
                                        className="ring-red-pantone/40 h-full w-full rounded-lg object-cover ring-1"
                                    />
                                </div>
